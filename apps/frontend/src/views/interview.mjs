...
export function renderInterview() {
  ...
  const speechLabels = { recording: "リアルタイム認識中", recognizing: "文字起こし中", recognized: "認識完了", failed: "認識失敗" };
  return layout(`<div class="interview-progress"><span>質問 ${qNo || 1} / ${total}</span><div><i style="width:${Math.min(100, ((qNo || 1) / total) * 100)}%"></i></div>${badge(recording ? "録音中" : state.busy ? "処理中" : "回答待ち", recording ? "red" : "green")}</div><section class="interview-card card"><div class="interviewer"><div class="interviewer-avatar">AI</div><div><strong>AI面接官</strong><small>${esc(state.settings?.speaker || "青山龍星")}</small></div><button class="icon-button sound" data-action="voice" aria-label="質問をもう一度読み上げる" ${!state.question || state.busy ? "disabled" : ""}>♫</button></div><blockquote>${esc(state.question?.text || "質問を準備しています…")}</blockquote></section><section class="card answer-card"><div class="card-title-row"><div><p class="eyebrow">YOUR ANSWER</p><h2>回答を入力</h2></div><div class="answer-heading-actions">${state.speechStatus !== "idle" ? badge(speechLabels[state.speechStatus] || state.speechStatus, state.speechStatus === "failed" ? "red" : "blue") : ""}${recording ? '<button class="recording-button" data-action="stop-recording"><span>■</span>録音を停止</button>' : `<button class="record-button" data-action="start-recording" ${state.busy ? "disabled" : ""}><span>●</span>音声で回答</button>`}</div></div><textarea id="answerDraft" aria-label="回答内容" placeholder="マイクで話すか、ここに回答を入力してください。" ${recording || state.busy ? "disabled" : ""}>${esc(state.answerDraft)}</textarea><div class="answer-controls"><span class="or">対応ブラウザでは話した内容をリアルタイムで入力します。認識結果は送信前に修正できます。</span><button class="primary submit-answer" data-action="submit-answer" ${canSubmit ? "" : "disabled"}>回答を送信 <span>→</span></button></div></section><div class="interview-footer"><button class="text-button danger-text" data-action="confirm-finish" ${state.busy || recording ? "disabled" : ""}>面接を終了する</button><span>回答は送信するまで保存されません</span></div>`, { eyebrow: "INTERVIEW IN PROGRESS", title: state.session?.condition?.theme || "面接練習" });
}
...
export function renderFeedback() {
  ...
  if (!fb && state.feedbackStatus === "failed") return layout(`<section class="card loading-card"><div class="confirm-icon">!</div><p class="eyebrow">ANALYSIS PAUSED</p><h2>フィードバックを開始できませんでした</h2><p>面接の質問と回答は練習履歴に保存されています。時間をおいて分析を再試行するか、履歴を確認してください。</p><div class="confirm-actions"><button data-screen="history">練習履歴を見る</button><button class="primary" data-action="retry-feedback" ${state.busy ? "disabled" : ""}>分析を再試行</button></div></section>`, { eyebrow: "FEEDBACK", title: "面接フィードバック" });
