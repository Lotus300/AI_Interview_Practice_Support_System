...
export function layout(content, { title = "", eyebrow = "" } = {}) {
  ...
  const brand = loggedIn
    ? '<button type="button" class="brand" data-screen="home" aria-label="ホームへ移動"><strong>AI面接練習</strong><span>Interview Practice</span></button>'
    : '<div class="brand"><strong>AI面接練習</strong><span>Interview Practice</span></div>';
  return `<header class="topbar">${loggedIn ? `<button class="icon-button" data-action="toggle-drawer" aria-label="メニューを開く" aria-expanded="${state.drawerOpen}">☰</button>` : '<div class="brand-mark">AI</div>'}${brand}<div class="topbar-user">${loggedIn ? `<span>${esc(state.user.name)}</span><span class="avatar">${esc(state.user.name.slice(0, 1))}</span>` : badge("デモ利用可能", "blue")}</div></header>
