...
フィードバック生成はジョブ型APIであるため、開始APIが返したjobIdを保持し、`GET /api/v1/jobs/{jobId}` を3秒間隔でポーリングする。`status=succeeded` になった後に `GET /api/v1/interview-sessions/{sessionId}/feedback` を1回呼び、結果を表示する。

`queued` または `running` の間はポーリングを継続し、`failed` では停止して再実行可能なエラーを表示する。画面離脱時はブラウザ側のポーリングを停止するが、Cloud Tasks上のジョブは継続する。再表示時は保存済みjobIdまたはセッションのfeedbackStatusから状態を再取得する。
