...
| 実行方式 | Cloud Tasks + Cloud Run内部ジョブAPI |
| API応答 | ジョブをqueuedで保存し、202 Acceptedを即時返却 |
| 画面状態確認 | `GET /jobs/{jobId}` を3秒間隔でポーリング |
...
フィードバックは、面接セッションに保存された実際の質問とユーザー回答を対応付けて生成する。固定の総評や、入力されていない経験・成果を推測してはならない。

### 13.1 評価対象

| データ | 評価への使用 |
|---|---|
| AIの質問 | 対応するユーザー回答がある場合の文脈として使用する |
| ユーザー回答 | 評価の主な根拠として使用する |
| 回答分析 | 抽象表現、矛盾候補、深掘り不足の根拠として使用する |
| AIの質問だけの発話 | ユーザー評価の根拠には使用しない |

フィードバックには次の監査用情報を保存する。

```ts
type FeedbackEvidence = {
  answerId: string | null;
  answerUtteranceId: string | null;
...
有効なユーザー回答が0件の場合は次のように処理する。

- `assessmentStatus` を `not_assessable` とする。
- `evaluatedAnswerCount` を `0` とする。
- `goodPoints` と `evidence` は空配列にする。
- 総評には、回答がないため会話内容に基づく評価ができないことを明記する。
- 改善回答例を捏造せず、次回の回答方法だけを案内する。
- 画面では「良かった点」を表示せず、「今回は評価できませんでした」と表示する。

### 13.3 回答ありの場合

- すべての保存済み回答を評価対象とする。
- 良かった点と改善点には、対応する回答の抜粋を含める。
- 画面に評価対象件数と、評価に使用した質問・回答を表示する。
- 回答に存在しない数値、成果、経験を補完または推測しない。
...
Cloud Runではリクエスト終了後のプロセス内バックグラウンド処理を保証できないため、Cloud Tasks方式を採用する。

| 方式 | 内容 | 採用判断 |
|---|---|---|
| API内で同期的にジョブ実行 | 採用しない | API応答がGemini処理完了まで返らないため |
| レスポンス後のプロセス内実行 | 採用しない | Cloud RunのCPU割当・インスタンス終了により完了が保証されないため |
| Cloud Tasksを利用 | 採用 | 再試行、OIDC認証、実行追跡が可能なため |

Cloud Tasksからの実行は少なくとも1回実行される可能性を前提とし、完了済みジョブの再呼び出しを安全に無視する。
